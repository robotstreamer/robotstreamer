...
import argparse
import json
from robotstreamer_utils import robot_util
import _thread
import traceback
import tempfile
import uuid
    ...
    try:
        ...
        if commandArgs.male:
            ...
            cmd = (
                "cat "
                + tempFilePath
                + " | espeak -p %d -v%s --stdout > %s"
                % (commandArgs.tts_pitch, voice, wavFile)
            )
            print("--------------", cmd)
            os.system(cmd)
        else:
            ...
            cmd = (
                "cat "
                + tempFilePath
                + " | espeak -p %d -v%s+f%d -s170 --stdout > %s"
                % (commandArgs.tts_pitch, voice, commandArgs.voice_number, wavFile)
            )
            print("--------------", cmd)
            os.system(cmd)
        ...
        cropCommand = "%s -i %s -ss 0 -to %d %s %s" % (
            commandArgs.ffmpeg_path,
            wavFile,
            maximumTTSTime,
            filterString,
            croppedWavFile,
...
def say(message, messageVolume, voice="en-us"):
    ...
    if commandArgs.audio_output_hardware_name is not None:
        audioOutputNumber = audio_util.getAudioPlayingDeviceByName(
            commandArgs.audio_output_hardware_name
        )
...
def getControlHost(apiHost="https://api.robotstreamer.com", camera_id="0"):

    url = apiHost + "/v1/get_service/rscontrol"
    response = robot_util.getNoRetry(url)
    ...
    if response is None:
        ...
        url = apiHost + "/v1/get_endpoint/rscontrol_robot/" + camera_id
        response = robot_util.getNoRetry(url)
        response = json.loads(response)
...
async def handleWesocketTester(key):
    ...
    while True:
        ...
        print(
            key.upper() + ": elapsed time since last acklowledgement (%s):" % key,
...
async def handleControlMessages():
    ...
    async with websockets.connect(url) as websocket:
        ...
        while True:
            ...
            elif j.get("command") and j.get(
                "key_position"
            ):  # apparently use of _thread for this on new os (2021) causes seg fault
                _thread.start_new_thread(
                    interface.handleCommand,
                    (j["command"], j["key_position"], j.get("command_price", 0)),
                )
        ...
        while True:
            ...
            if j.get("command") == "RS_PONG":
                ...
                if ("tts" in j) and j["tts"] == True:
                    ...
                    if len(messagesToTTS) < commandArgs.free_tts_queue_size or (
                        ("tts_price" in j) and (j["tts_price"] >= 0.01)
                    ):
...
def runPeriodicTasks():
    ...
    if robot_util.getSoundQueueSize() < 1:
        if (
            len(messagesToTTS) > 0
            and numActiveEspeak == 0
            and robot_util.getNumActiveSounds() == 0
        ):
        ...
        if numActiveEspeak == 0 and robot_util.getNumActiveSounds() == 0:
            _thread.start_new_thread(
                robot_util.aplayFile, (robot_util.popSoundQueue(),)
            )
...
def main():
    try:
        ...
        print(
            "warning: google tts module does not exist so google tts will not work. you can install the python module if you want to use google tts."
        )
    ...
    parser.add_argument(
        "--play-nontts-softly", dest="play_nontts_softly", action="store_true"
    )
    parser.add_argument(
        "--enable-ping-pong", dest="enable_ping_pong", action="store_true"
    )
    ...
    parser.add_argument(
        "--disable-volume-set", dest="disable_volume_set", action="store_true"
    )
    ...
    parser.add_argument(
        "--kill-on-failed-connection",
        dest="kill_on_failed_connection",
        action="store_true",
    )
